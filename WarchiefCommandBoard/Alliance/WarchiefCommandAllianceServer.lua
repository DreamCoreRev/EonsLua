-- WarchiefCommandAllianceServer.lua
...
if AIO.IsMainState() then
    ...
    local availableQuests = {
        { id = 27721, title = "Uldum : L'appel du chef de guerre" },
        { id = 27726, title = "Hyjal : Aux portes de la guerre" },
        { id = 27727, title = "Maelström : Contre les forces du chaos" },
    }

    local teleportData = {
        [27721] = { mapId = 829,  x = -11018.59, y = -1263.54,  z = 13.52,    o = 4.85,    message = "Vous avez été téléporté à Uldum !" },
        [27726] = { mapId = 1,    x = 5534.08,   y = -3624.69,  z = 1567.04,  o = 5.39048, message = "Vous avez été téléporté au Mont Hyjal !" },
        [27727] = { mapId = 852,  x = 854.924,   y = 1080.96,   z = -12.5196, o = 4.54772, message = "Vous avez été téléporté au Maelström !" },
    }

    function WarchiefCommandAllianceHandlers.AcceptQuest(player, questId)
...
local function OnGossipHello(event, player, gameObject)
    ...
    AIO.Handle(player, "WarchiefCommandAllianceHandler", "OpenInterface")
    player:GossipComplete()
end

-- Remplacez 206111 par l'ID de votre GameObject Alliance.
RegisterGameObjectGossipEvent(206111, 1, OnGossipHello)
