...
function SHOP_UI.MainFrame_Create()
	...
	-- Passer au-dessus de toute l'UI du jeu (bags, map, GameMenu, etc.)
	-- Utiliser "FULLSCREEN_DIALOG" pour passer aussi au-dessus du GameMenuFrame (Échap)
	shopFrame:SetFrameStrata("DIALOG")

	-- Rendre le frame déplaçable à la souris
	shopFrame:SetMovable(true)
	shopFrame:EnableMouse(true)
	shopFrame:RegisterForDrag("LeftButton")
	shopFrame:SetScript("OnDragStart", function(self)
		self:StartMoving()
	end)
	shopFrame:SetScript("OnDragStop", function(self)
...
StaticPopupDialogs["CONFIRM_STORE_PURCHASE"] = {
	text = "Êtes-vous sûr de vouloir acheter %s?",
	button1 = "Oui",
	button2 = "Non",
...
function SHOP_UI.ServiceBoxes_Update()
	...
	for i, serviceData in ipairs(services) do
		if i >= startIndex and i <= endIndex then
			...
			service.BannerText:SetFormattedText("|cffffffffPROMO: %i%%|r", discountPct)
...
function SHOP_UI.ModelFrame_Create(parent)
	...
	-- Le MODEL_FRAME hérite de la strata du parent mais on la force aussi pour être sûr
