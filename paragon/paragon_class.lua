...
---
--- Saves all paragon data synchronously (blocking) to the database.
---
--- FIX: CharDBExecute (utilisé dans Save()) est asynchrone. Au moment du
--- logout, la requête peut être annulée avant exécution si la session se ferme
--- trop vite. SaveSync() utilise CharDBQuery (synchrone) via les méthodes
--- SaveParagonByCharacterSync / SaveParagonByAccountSync du Repository pour
--- garantir l'écriture effective en base avant que le handler Lua se termine.
---
--- Utilisé par Hook.OnPlayerLogout et Hook.OnPlayerSave.
---
function Paragon:SaveSync()
    if (Config:GetByField("LEVEL_LINKED_TO_ACCOUNT") == "1") then
        Repository:SaveParagonByAccountSync(self.account, self.level, self.exp.current)
    else
        Repository:SaveParagonByCharacterSync(self.guid, self.level, self.exp.current)
    end
    -- SaveParagonCharacterStat utilise desormais CharDBQuery (synchrone)
    -- pour garantir l'ecriture des stats avant la fin du handler logout.
    Repository:SaveParagonCharacterStat(self.guid, self.statistics)
end
...
function Paragon:SetLevel(level)
    ...
    if previous_level ~= level then
        ...
        -- Use self._player if set by the hook (avoids GetPlayerByGUID which is not
        -- available in all Lua map states in Eluna TrinityWotlk)
        if Mediator then
            local player_obj = self._player or (GetPlayerByGUID and GetPlayerByGUID(GetPlayerGUID(self.guid)))
